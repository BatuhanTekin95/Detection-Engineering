...
In this room, I focused on how threat intelligence and previous incident findings can be turned into actionable detection logic. The scenario shows how IOCs from a previous intrusion can be used to create detections that may help identify similar activity in the future.

This helped me understand that detection engineering is not only about enabling default alerts. A useful detection should be created, tested, reviewed, and improved over time so that it provides meaningful visibility for SOC analysts.
...
The main point I took from this section is that IOCs are useful for quick detection and scoping, but stronger detection strategies should also include behaviour-based detections.
...
I found Sigma useful because it helps detection rules become more structured, reusable, and easier to share.
...
This showed me that a detection rule should not only contain search logic. It should also include context, required log sources, possible false positives, and severity information so the analyst can understand why the alert matters.
...
This helped me understand why detection engineering requires balance. A detection should be specific enough to identify suspicious activity, but also practical enough to remain useful when attackers make small changes.

## Analyst Notes

The most important point I learned from this room is that tactical detection connects threat intelligence with practical SOC monitoring.
...
This room also helped me understand the value of Sigma in detection engineering. Sigma provides a structured way to document detection logic and makes it easier to adapt rules across different SIEM platforms.
...
The important point I took from this section is that tools like Uncoder can make the detection engineering process faster, but they do not remove the need for analyst review.
...
This helped me understand that vulnerability-based detections often depend on process creation telemetry and command-line visibility. Without proper logging, this type of activity may be difficult to detect.

## Log4j Suspicious Shell Detection Example

Another example was related to Log4j exploitation.

The Sigma rule focused on suspicious shell processes spawned by a Java process. I found this example important because it is more behaviour-based than simple IOC matching. In many environments, a Java process spawning shells or administrative tools can be suspicious and worth investigating.
...
This part also helped me see the difference between copying a detection rule and engineering a detection. A copied rule may generate alerts, but an engineered detection should be tested, tuned, documented, and useful during investigation.

## Detection Engineering Takeaway

The most important point I learned here is that Sigma and tools like Uncoder can make detection development faster, but they are not a replacement for analyst thinking.
...
This section showed me how public detection content can be transformed into practical SIEM detections, but it also showed why every detection should be validated before being trusted in production.
...
This helped me understand that detection engineering is not always about malware, IOCs, or exploit behaviour. Sometimes, a simple and well-placed monitoring rule can provide strong visibility into suspicious activity.
...
From an analyst point of view, I found this useful because it creates a simple detection opportunity. Instead of waiting for a known IOC or signature, the organization can place sensitive-looking resources and monitor unexpected access.
...
This helped me understand how Windows logs object access activity and how these events can be used during SOC investigations.
...
This helped me understand that detection engineering should not stop after writing a rule. A detection should be tested against realistic activity and improved based on the results.
...
The main lesson I took from this section is that purple teaming helps connect offensive activity with defensive visibility. If a technique is detected, it confirms that the current visibility is working in that area. If it is missed, that gap can be used to improve logging, detection logic, or alerting coverage.


## Tripwire Analyst Notes

The main point I learned from this section is that tripwires can add another layer of detection without depending only on known malware indicators.
...
This section helped me understand that detection engineering can also include deception-based ideas. A simple hidden file, honeypot, or monitored folder can help detect suspicious behaviour when attackers or malware interact with something they should not touch.
...
This room helped me understand how an organization can improve its detection capability by using information that is already available in its own environment.
...
I also understood the importance of layered defense. A SOC should not depend on only one type of detection or one security product. IOC-based detections, behaviour-based detections, file-based tripwires, object access auditing, and purple team validation can all work together to increase visibility and improve the chances of detecting suspicious activity.

From an analyst point of view, the most important lesson was that detections should be adapted to the environment. Default rules can provide a starting point, but they should be reviewed, tested, tuned, and improved based on real activity and organizational needs.

This room also showed me that purple team tactics are useful for validating detection coverage. By simulating attacker activity and reviewing the generated logs, analysts can understand which techniques were detected, which ones were missed, and where detection gaps still exist.

Overall, this room made it clear to me that detection engineering is not a one-time task. It needs continuous testing, tuning, and improvement as the environment and attacker behaviour change. A good detection strategy should combine multiple layers, use available telemetry effectively, and be reviewed regularly to stay useful.

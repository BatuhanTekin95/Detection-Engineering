...
In this room, I focused on **Sigma**, an open-source generic signature language used to write detection rules in a structured and platform-independent format.
...
This helped me understand that Sigma is not only a rule format. It is also a way to standardise detection engineering work across different platforms.
...
This helped me understand that Sigma rules are not only about detection logic. They also require clean formatting, because a small indentation or syntax mistake can break the rule.
...
> This section helped me understand how Sigma rules are structured and how detection logic can be built using fields, values, modifiers, conditions, and filters.
...
This showed me how Sigma can be used as a platform-independent detection format and then converted into a query that can be tested in a specific SIEM environment.
...
> This showed me that file creation detections need careful filtering. A Sigma rule should focus on the suspicious file creation behaviour described in the scenario, while avoiding unrelated system-generated `.txt` files.
...
This room helped me understand how Sigma can be used to write structured and reusable detection rules.

The most important points I learned were:
...
From a SOC analyst point of view, the most important lesson was that Sigma is not only about writing rules. A useful detection also requires understanding the log source, choosing the right fields, converting the rule correctly, and validating the result in the SIEM.
...
This room helped me understand how Sigma rules can support detection engineering by making detection logic easier to write, share, convert, and validate across different SIEM environments.

> From an analyst point of view, I learned that a Sigma rule is only useful when it is tested against real logs and adjusted to match the actual field names, log sources, and backend configuration of the environment.
